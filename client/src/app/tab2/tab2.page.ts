  ...
  takePicture() {
    ...
    this.camera.getPicture(options).then((imageData) => {
      ...
      .then((response) => {
        ...
        // this.logService.log('before wait');
        // return wait();
      })
      .then(() => {
        // this.logService.log('after wait');
  ...
  upload() {
    ...
    return this.uploadedImage;
      ...
      .then((cloudResponse) => {
        ...
        this.http.post('http://http://ec2-3-17-178-179.us-east-2.compute.amazonaws.com:8080/clothingImage/1', {
          "response": {
            "url": url,
          }
        }, { responseType: 'text' }).subscribe((response) => {
          this.uploadedImage.logService.log(response);
          localStorage.setItem('response', response);         
        })
        return wait(); 
      })
